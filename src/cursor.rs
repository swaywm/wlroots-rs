...
use wlroots_sys::{wlr_cursor, wlr_cursor_create, wlr_cursor_destroy, wlr_xcursor,
                  wlr_xcursor_theme, wlr_cursor_set_xcursor, wlr_xcursor_theme_get_cursor, wlr_xcursor_theme_load};
...
impl Cursor {
    ...
    pub fn set_xcursor(&mut self, xcursor: XCursor) {
        unsafe {
            wlr_cursor_set_xcursor(self.cursor, xcursor.into_raw())
        }
    }
}
...
impl XCursorTheme {
    ...
    pub fn get_cursor(&self, name: String) -> Option<XCursor> {
        ...
        let xcursor = unsafe {
            wlr_xcursor_theme_get_cursor(self.theme, name_str.as_ptr())
        };
        if xcursor.is_null() {
            None
        } else {
