use wlroots::{compositor, cursor, input::pointer, seat::Capability, wlroots_dehandle};
...
impl pointer::Handler for PointerHandler {
    ...
    fn on_button(
        ...
        if let Some(shell) = mapped_shells.back() {
            ...
            seat.keyboard_notify_enter(surface, &mut keycodes, &mut modifier_masks);
        }
    }
