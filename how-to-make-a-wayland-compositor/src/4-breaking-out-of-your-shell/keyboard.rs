use wlroots::{
    compositor, input::keyboard, seat::Capability, wlr_key_state::WLR_KEY_PRESSED, wlroots_dehandle,
...
impl keyboard::Handler for KeyboardHandler {
    ...
    fn modifiers(&mut self, compositor_handle: compositor::Handle, keyboard_handle: keyboard::Handle) {
        ...
        #[dehandle]
        let keyboard = keyboard_handle;
        let CompositorState { ref seat_handle, .. } = compositor.data.downcast_mut().unwrap();
        #[dehandle]
        let seat = seat_handle;
        seat.set_keyboard(keyboard.input_device());
